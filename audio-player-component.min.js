(() => {
  ...
  var m = {
    ...
    constructor() {
      super(), this.#s = this.attachShadow({ mode: "open" }), this.#e = this.getAttribute("title"), this.#a = this.getAttribute("artist"), this.#r = this.getAttribute("image") || "", this.#i = this.getAttribute("compact") !== null, this.#n = this.getAttribute("blurred") !== "false" || !(this.#e && this.#a), this.#l = this.getAttribute("dark") !== null, this.#d = this.getAttribute("colour") || "#3FA9F5";
      let e = Number(this.getAttribute("min-height") ?? 280);
      this.#p = Number.isSafeInteger(e) ? e : 280;
      let a = this.getAttribute("sources")?.split(",") ?? [];
      this.loadAudio(a), this.#s.appendChild(t.createElement({ tagName: "style", textContent: this.styleCss })), this.createPlayer();
    }
    static padTime(e) {
      return e < 10 ? `0${e}` : e.toString();
    }
    static createElement({ tagName: e = "div", className: a = "", ...o } = {}) {
    ...
    loadAudio(e) {
      for (let a of e) {
        let o = a.split(".").pop(), i = m[o ?? "mp3"];
        if (!i) {
          console.warn(`Could not find MIME-type for '${o}' type. Skipping.`);
          continue;
        }
        this.#t.appendChild(
          t.createElement({
    ...
    addOverlayToContainer(e) {
      ...
      let o = t.createElement({ className: "tap--overlay" }), i = t.createElement({ className: "tap--metadata" }), r = t.createElement({
        tagName: "span",
      ...
      i.appendChild(r), i.appendChild(s), e.appendChild(o), e.appendChild(i);
    }
    createPlayer() {
      let e = t.createElement({ className: "tap--container" });
      this.#i || (e.style.minHeight = `${this.#p}px`), this.addOverlayToContainer(e);
      let a = t.createElement({ className: "tap--controls" });
      this.#l && a.classList.add("dark");
      let o = t.createElement({ className: "tap--progress--bar" }), i = t.createElement({ className: "tap--progress--playhead" }), r = t.createElement({ className: "tap--progress--timestamp" }), s = t.createElement({ tagName: "button", className: "tap--button" });
      s.innerHTML = t.svgPlay, i.style.backgroundColor = this.#d;
      let l = () => {
        let n = t.padTime(Math.floor(this.#t.currentTime / 60)), d = t.padTime(Math.floor(this.#t.currentTime % 60)), u = t.padTime(Math.floor((this.#t.duration || 0) / 60)), g = t.padTime(Math.floor((this.#t.duration || 0) % 60));
        r.innerHTML = `${n}:${d}<br/>${u}:${g}`, i.style.width = `${this.#t.currentTime * 100 / (this.#t.duration || 1)}%`;
      };
      s.onclick = () => {
        if (!this.#o) {
          this.#t.play(), this.#o = !0, s.innerHTML = t.svgPause;
          return;
        }
        this.#t.pause(), this.#o = !1, s.innerHTML = t.svgPlay;
      };
      let h = (n) => {
      ...
      if (o.onmousedown = () => {
        ...
        p = !1;
      }, o.onmousemove = (n) => p && h(n), o.onclick = h, o.appendChild(i), a.appendChild(s), a.appendChild(o), a.appendChild(r), l(), this.#t.ondurationchange = l, this.#t.ontimeupdate = l, this.#t.onended = () => {
        this.#t.currentTime = 0, this.#o = !1, s.innerHTML = t.svgPlay;
      }, this.#t.style.display = "none", this.#i) {
        this.#s.appendChild(a);
        return;
      }
      e.appendChild(a), this.#s.appendChild(e);
    }
  };
